...
func (a *aws) Replicate(amiName string, targetRegion string) (pulumi.RunFunc, error) {
	r := replicateRequest{
		amiName,
		targetRegion}
...
func (r replicateRequest) runFunc(ctx *pulumi.Context) error {
	return replicateAMI(ctx, r.amiName, r.targetRegion)
}

func replicateAMI(ctx *pulumi.Context, amiName, targetRegion string) error {
	...
	if *amiInfo.Region == targetRegion {
		return nil
	}
