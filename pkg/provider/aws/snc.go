...
func (r sncEphemeralRequest) sncEphemeralRunFunc(ctx *pulumi.Context) error {
	...
	arch := string(*r.arch)
	amiName := fmt.Sprintf("%s-%s",
		*amiBaseName, arch)
	ctx.Export(outAMIName, pulumi.String(amiName))
	ctx.Export(outAMIArch, pulumi.String(arch))
	extractExecution, err := bundle.Extract(ctx, amiName, r.bundleURI, r.shasumURI, "aws")
	if err != nil {
		return err
	}
	_, err = bucketEphemeral(ctx, &amiName)
	if err != nil {
		return err
	}
	ctx.Export(outBucketName, pulumi.String(amiName))
